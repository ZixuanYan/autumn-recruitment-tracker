...
(function (root) {
  ...
  function groupsOf(list) {
    if (typeof groupRecordsByCompany === 'function') return groupRecordsByCompany(list);
  ...
  function buildShareReport(records, options = {}) {
    ...
    const cities = cityStats.map(row => [row.city || '未填写', row.total]);
    const namedCities = cities.filter(([name]) => name !== '未填写').slice(0, 8);
    const otherCount = cities.filter(([name]) => name !== '未填写').slice(8).reduce((sum, [, count]) => sum + count, 0);
    if (otherCount) namedCities.push(['其他', otherCount]);
    const unknownCity = cities.find(([name]) => name === '未填写');
    ...
    const cityHtml = namedCities.length ? namedCities.map(([name, count]) => `<li><span>${esc(name)}</span><b>${count}</b></li>`).join('') : '<li>未填写 <b>0</b></li>';
    const typeHtml = companyTypes.map(type => `<li><span>${esc(type)}</span><b>${typeCounts[type]}</b></li>`).join('');
    const rows = sorted.map((r, i) => {
      const company = `${r.company || '未填写'}${r.orgUnit ? ` · ${r.orgUnit}` : ''}`;
      const tl = Array.isArray(r.timeline) ? r.timeline : [];
      const done = tl.length && tl[tl.length - 1].done === true;
      return `<tr><td data-label="编号">${i + 1}</td><td data-label="公司 / 机构">${esc(company)}${r.companyType ? ` <small class="tag">${esc(r.companyType)}</small>` : ''}</td><td data-label="岗位">${esc(r.position || '未填写')}</td><td data-label="城市">${esc(r.city || '未填写')}</td><td data-label="投递日期">${esc(day(r.applicationDate))}</td><td data-label="当前阶段"><span class="stage">${esc(currentStage(r))}</span>${done ? ' <small class="done">已完成</small>' : ''}</td></tr>`;
    }).join('');
