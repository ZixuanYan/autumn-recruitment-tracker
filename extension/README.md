# 秋招求职与简历助手（Chrome/Edge 扩展 v5.4.0）
...
- 扩展：v5.4.0（数据位补 `tabular-nums`、两个样式上下文补 `prefers-reduced-motion` 总开关、头部硬规则注释与代码对账；阶段/企业性质/归一化/默认简历消费仓库根 `shared/` 的单一事实源；Side Panel + 可拖拽胶囊 + 设计令牌化样式自 v5.0.0 起）
