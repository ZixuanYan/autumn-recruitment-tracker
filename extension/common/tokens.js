...
(() => {
  ...
  const TOKENS = {
    ...
    // 唯一装饰性强调色。
    // 刻意**不**沿用网页版的 --accent（那是蓝紫色）：蓝紫渐变正是插件旧版「AI 感」的主要来源，
    // 旧胶囊与标题栏用的都是同一色系的渐变，换成低饱和的橙更符合工具类界面的克制。
    // 代价是两端品牌色暂不统一。若将来要统一，应当两端一起改设计语言，而不是把插件改回蓝紫。
    // test/extension-ui.js 有一份旧配色黑名单会拦下任何回归；该守卫不区分注释与代码，
    // 所以这里刻意只写「蓝紫色」而不写出具体色值，否则连这段说明都会被自己拦下。
    accent: '#e8552d',
    accentHover: '#cf4a26',
    accentSoft: 'rgba(232,85,45,.12)',

    // 语义色（功能必需，非装饰）：危险动作与识别告警。
    // 旧版这里散落着四套互不相干的彩色——绿色的保存按钮、蓝紫的计数徽标、红色的丢弃按钮、
    // 琥珀色的告警提示，谁也不服谁；现在收敛为 accent / danger / warn 三色。
    // （同上一样的理由：这里只写颜色名不写 hex，否则会被 extension-ui.js 的黑名单守卫拦下）
    danger: '#c0392b',
