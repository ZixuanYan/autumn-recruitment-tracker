 ...
 *
 * 为什么要有这个文件：收录表单的 HTML 模板、字段 id、下拉选项生成、置信提示渲染与保存链路
 * 都在这里，Side Panel 的「一键收录」段直接复用（v5.0.0-v5.5.0 期间 content script 的迷你
 * 收录卡片也用它；v5.6.0 迷你卡片随页面胶囊退役后只剩面板一个消费方）。表单运行在扩展页面
 * 上下文，与宿主页 DOM 隔离，抽成共享模块后字段增删只改一处，避免漂移。
 *
...
(() => {
  ...
  /**
   * 表单的组件样式（v5.6.0 起只有面板一个消费方）。
   * 和 html() 放在一起是刻意的：模板与样式同源，改一处就不会出现模板与样式脱节。
   * 全部走 var(--aja-*)，因此深浅主题自动适配；调用方负责先注入 tokensToCssVars。
   * 选择器一律用 .capture-form 作用域而不是端上专有的 id，
