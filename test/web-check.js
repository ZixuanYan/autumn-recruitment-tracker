...
check('日历视图接线：路由钩子 / 中央重渲 / 点格选中 / 日程流与 chip 直达抽屉（v4.27.0 ~ v4.30.0）', () => {
  ...
  assert.ok(/calSelectedDate = iso === localDateInput\(new Date\(\)\) \? '' : iso/.test(html), '今天用空串表示默认态，其他日期保持单日选择');
  assert.ok(/calViewYear = d\.getFullYear\(\);[\s\S]*?calViewMonth = d\.getMonth\(\);/.test(extractFunction(html, 'calSelectDay')), '点跨月补位日期要同步切换年月');
  ...
  assert.ok(/const activeDate = calSelectedDate \|\| today/.test(extractFunction(html, 'renderCalFlow')), '日程右栏默认今天，选中后只读对应日期');
  assert.ok(!/for \(let off = 0; off < 7/.test(extractFunction(html, 'renderCalFlow')), '单日详情不得重新混入未来 7 天');
  ...
  assert.ok(/calendarShiftMonth\(delta\) \{[\s\S]*?Math\.min\(active\.getDate\(\)/.test(html), '翻月应选中目标月同一日，避免月历与右栏日期脱节');
  assert.ok(/addEventListener\('keydown'[\s\S]*?\['Enter', ' '\]/.test(html), '日期格支持 Enter / Space 键盘选择');
