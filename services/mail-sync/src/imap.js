...
function planFetch(mailbox, meta, sinceDays, uidFrom, retryOnly = false) {
  const uidValidity = Number(mailbox && mailbox.uidValidity) || 0;
  const forced = Number(uidFrom) || 0;
  if (forced > 0) return { mode: 'uid', startUid: forced, retryUids: [], retryOnly: false, uidValidity, forced: true, resetWatermark: false };
  ...
  if (!lastUid || validityChanged) {
    ...
    return { mode: 'since', since, retryUids: [], retryOnly: false, resetWatermark: validityChanged, uidValidity, forced: false };
  }
  const retryUids = Array.isArray(meta && meta.retryUids)
    ? meta.retryUids.map(v => Number(v) || 0).filter(v => v > 0).slice(0, 200)
    : [];
  return { mode: 'uid', startUid: lastUid + 1, retryUids, retryOnly: Boolean(retryOnly), uidValidity, forced: false, resetWatermark: false };
}
...
async function fetchMessages(client, plan, maxPerRun) {
  ...
  if (plan.mode === 'uid') {
    const seen = new Set();
    for (const uid of (plan.retryUids || [])) {
      for await (const msg of client.fetch(String(uid), query, { uid: true })) {
        if (Number(msg.uid) === uid && !seen.has(uid)) { seen.add(uid); out.push(msg); }
        if (out.length >= cap) return out;
      }
    }
    if (plan.retryOnly) return out;
    ...
    for await (const msg of client.fetch(`${plan.startUid}:*`, query, { uid: true })) {
      if (Number(msg.uid) < plan.startUid || seen.has(Number(msg.uid))) continue;
      out.push(msg);
