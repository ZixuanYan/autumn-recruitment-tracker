...
// 现在 cron 改细到每小时尝试，若 0 仍按旧语义解释，所有没显式设置过这一项的部署
// 会从每 12 小时**静默**变成每小时——AI 调用量最多 12 倍，而用户什么都没做。
// 把 0（含未设置）解释为"用默认 12 小时"，已有部署的实际频率不变；想更频繁就显式填 1 / 2 / 6。
...
// 不留容差的话，把 minIntervalHours 设成与 cron 周期相同（都是 1 小时）会得到
// elapsed = 1h − 2min < 1h → 本次跳过 → 实际变成每 2 小时一次，而且完全静默：
// 日志里只有一行"距上次运行不足 1 小时"，看起来像是配置没生效。
// 10 分钟足以覆盖单轮耗时与 GitHub 定时的小幅抖动，又不会把"每小时"实质缩短太多。
