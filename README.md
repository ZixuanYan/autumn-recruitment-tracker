...
- 左侧独立入口：总览（统计 + 洞察 + 未来安排）/ **日历**（v4.27.0，月历看每天的开始时间与截止时间，可导出 .ics；v4.28.0 起点日期展开当日明细、已完成聚合为浅绿「N 条已完成」、周末列浅底、逾期日红点标记、标题行显示本月汇总）/ 投递记录（表格 / 看板双视图台账，v4.8.0 从总览独立成页）/ 我的简历 / 工具 / 邮件提醒（可选）
- 工具视图集中管理数据安全与云同步，卡片实时显示快照数量与同步连接状态
- 今日投递统计、洞察（阶段分布 / 转化漏斗 / 需要关注 / 城市分布 / 企业性质 / 投递节奏 / 各阶段停留）和未来安排在总览；搜索、阶段筛选、排序、**同企业收纳**（v4.25.0 起组头是公司卡片：公司名 + 岗位数 + 一排岗位胶囊，折叠时就是多岗位扫描视图；v4.26.0 起可一键全部折叠 / 展开）与表格 / 看板切换在投递记录页
