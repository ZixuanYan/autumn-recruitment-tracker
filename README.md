...
| 内容 | 适用场景 | 下载 |
| --- | --- | --- |
| 秋招求职与简历助手 插件 v5.6.0 | 电脑 Chrome/Edge 网申填表 + 一键收录 + 暂存箱 | [前往插件目录](https://github.com/ZixuanYan/autumn-recruitment-tracker/tree/main/extension) |
...
- 左侧独立入口：总览（统计 + 洞察）/ **日历**（v4.27.0，月历看每天的开始时间与截止时间；v4.28.0 起点日期展开当日明细、已完成聚合为浅绿「N 条已完成」；v4.29.0 起**未来安排迁到本页顶部**、宽屏左月历右明细两栏）/ 投递记录（表格 / 看板双视图台账，v4.8.0 从总览独立成页）/ 我的简历 / 工具 / 邮件提醒（可选）
- 工具视图集中管理数据安全与云同步，卡片实时显示快照数量与同步连接状态
- 今日投递统计与洞察（阶段分布 / 转化漏斗 / 需要关注 / 城市分布 / 企业性质 / 投递节奏 / 各阶段停留）在总览；**未来安排与月历在日历页**（v4.29.0 起未来安排从总览迁入）；搜索、阶段筛选、排序、**同企业收纳**（v4.25.0 起组头是公司卡片：公司名 + 岗位数 + 一排岗位胶囊，折叠时就是多岗位扫描视图；v4.26.0 起可一键全部折叠 / 展开）与表格 / 看板切换在投递记录页
